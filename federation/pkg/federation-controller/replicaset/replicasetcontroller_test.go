...
import (
	...
	_ "k8s.io/kubernetes/federation/client/clientset_generated/federation_release_1_4"
...
func TestReplicaSetController(t *testing.T) {
	...
	fmt.Printf("===: %v\n", rs)
	rs, err := fedclientset.Extensions().ReplicaSets(apiv1.NamespaceDefault).Create(rs)
	fmt.Printf("===: %v, %v\n", rs, err)
	fedrswatch.Add(rs)
